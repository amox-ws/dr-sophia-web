          ...
          {heroSlides.map((slide) => (
            <CarouselItem key={slide.id}>
              ...
              <div className="relative h-[calc(100vh-4rem)] md:h-[calc(100vh-5rem)] flex items-center text-white overflow-hidden">
                ...
                {/* 1. We removed `filter brightness-[.3]`. This kills mobile performance. */}
                {/* 2. We use a separate overlay div with `bg-black/60` instead. */}
                <div className="absolute inset-0 w-full h-full">
                  ...
                  {/* Dark Overlay (Replaces brightness filter for speed) */}
                  <div className="absolute inset-0 bg-black/60" />
                </div>

                {/* Decorative glowing elements - HIDDEN ON MOBILE to save GPU */}
